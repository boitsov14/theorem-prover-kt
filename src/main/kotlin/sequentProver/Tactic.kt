...
object AXIOM : ITactic {
	...
	object ApplyData : IApplyData {
		override val tactic = AXIOM
	}
}
...
enum class BinaryTactic : ITactic {
	...
	data class ApplyData(override val tactic: BinaryTactic, val fml: Formula) : IApplyData {
		fun applyTactic(sequent: Sequent): Pair<Sequent, Sequent> =
			tactic.applyTactic(sequent, fml).first to tactic.applyTactic(sequent, fml).second
	}
	...
	fun getAvailableFml(sequent: Sequent): Formula? = when (this) {
		...
		IFF_RIGHT -> sequent.conclusions.firstOrNull {
			it is IFF && IMPLIES(
				it.leftFml,
				it.rightFml
			) !in sequent.conclusions && IMPLIES(it.rightFml, it.leftFml) !in sequent.conclusions
		}
	}
	...
	private fun applyTactic(sequent: Sequent, fml: Formula): Pair<Sequent, Sequent> = when (this) {
		...
		IMPLIES_LEFT -> {
			...
			val newSequent1 = sequent.copy(
				assumptions = sequent.assumptions - fml, conclusions = sequent.conclusions + fml.leftFml
			)
